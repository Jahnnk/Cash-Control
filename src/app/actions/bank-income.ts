...
export async function saveBankIncomeItems(
  ...
  await db.execute(sql`
    ...
    WHERE business_id = ${bId} AND date = ${date} AND is_special_loan = false
  `);
...
export async function updateBankIncomeItem(id: string, data: { amount?: number; clientId?: string | null; note?: string }) {
  const bId = await activeBusinessId();
  // Cross-tenant guard + bloqueo de préstamos del socio:
  // los items con is_special_loan=true se gestionan en /atelier/prestamos-socio.
  const target = (await db.execute(sql`
    SELECT date::text as date, is_special_loan
    FROM bank_income_items WHERE id = ${id} AND business_id = ${bId}
  `)).rows[0] as { date: string; is_special_loan: boolean } | undefined;
...
export async function getBankIncomeItems(date: string) {
  ...
  const result = await db.execute(sql`
    ...
    WHERE bi.business_id = ${bId} AND bi.date = ${date} AND bi.is_special_loan = false
