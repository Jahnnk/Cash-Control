...
export async function getReconciliation(startDate: string, endDate: string) {
  ...
  const daily = await db.execute(sql`
    SELECT
      ...
      COALESCE((SELECT SUM(amount) FROM expenses WHERE business_id = ${bId} AND date = dr.date AND payment_method NOT IN ('efectivo','pendiente_atelier') AND is_special_loan = false), 0) as bank_expenses,
      COALESCE(dr.bank_income, 0) - COALESCE((SELECT SUM(amount) FROM expenses WHERE business_id = ${bId} AND date = dr.date AND payment_method NOT IN ('efectivo','pendiente_atelier') AND is_special_loan = false), 0) as bank_net,
  ...
  const totals = await db.execute(sql`
    SELECT
      ...
      COALESCE((SELECT SUM(amount) FROM expenses WHERE business_id = ${bId} AND date >= ${startDate} AND date <= ${endDate} AND payment_method NOT IN ('efectivo','pendiente_atelier') AND is_special_loan = false), 0) as total_bank_expenses,
      COALESCE((SELECT SUM(amount) FROM expenses WHERE business_id = ${bId} AND date >= ${startDate} AND date <= ${endDate} AND payment_method = 'efectivo' AND is_special_loan = false), 0) as total_cash_expenses,
      COALESCE((SELECT SUM(amount) FROM expenses WHERE business_id = ${bId} AND date >= ${startDate} AND date <= ${endDate} AND is_special_loan = false), 0) as total_expenses,
