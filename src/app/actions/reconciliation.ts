...
export async function getReconciliation(startDate: string, endDate: string) {
  ...
  const daily = await db.execute(sql`
    SELECT
      ...
      COALESCE((SELECT SUM(amount) FROM expenses WHERE business_id = ${bId} AND date = dr.date AND payment_method NOT IN ('efectivo','pendiente_atelier','socio') AND is_special_loan = false AND is_internal_transfer = false AND archived = false), 0) as bank_expenses,
      COALESCE(dr.bank_income, 0) - COALESCE((SELECT SUM(amount) FROM expenses WHERE business_id = ${bId} AND date = dr.date AND payment_method NOT IN ('efectivo','pendiente_atelier','socio') AND is_special_loan = false AND is_internal_transfer = false AND archived = false), 0) as bank_net,
  ...
  const totals = await db.execute(sql`
    SELECT
      ...
      COALESCE((SELECT SUM(amount) FROM expenses WHERE business_id = ${bId} AND date >= ${startDate} AND date <= ${endDate} AND payment_method NOT IN ('efectivo','pendiente_atelier','socio') AND is_special_loan = false AND is_internal_transfer = false AND archived = false), 0) as total_bank_expenses,
