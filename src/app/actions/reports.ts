  ...
  const totals = await db.execute(sql`
    SELECT
      ...
      COALESCE((
        ...
        WHERE business_id = ${bId} AND date >= ${startDate} AND date <= ${endDate} AND is_fonavi_reimbursement = false
      ), 0) as total_income,
      ...
      COALESCE((
        ...
        FROM expenses WHERE business_id = ${bId} AND date >= ${startDate} AND date <= ${endDate}
      ), 0) as total_expenses,
      COALESCE((
        SELECT SUM(amount) FROM bank_income_items
