  ...
  const totals = await db.execute(sql`
    SELECT
      ...
      COALESCE((
        ...
        WHERE business_id = ${bId} AND date >= ${startDate} AND date <= ${endDate} AND is_fonavi_reimbursement = false AND is_special_loan = false
      ), 0) as total_income,
      ...
      COALESCE((
        ...
        FROM expenses WHERE business_id = ${bId} AND date >= ${startDate} AND date <= ${endDate} AND is_special_loan = false
      ), 0) as total_expenses,
      COALESCE((
        SELECT SUM(amount) FROM bank_income_items
...
export async function getDailyBreakdown(month: string, type: "byte" | "income" | "expense") {
  ...
  if (type === "byte") {
    const result = await db.execute(sql`
      ...
      FROM daily_records
      WHERE business_id = ${bId} AND date >= ${startDate} AND date <= ${endDate} AND COALESCE(byte_total, 0) > 0
      ORDER BY date ASC
    `);
    return result.rows;
  } else if (type === "income") {
    const result = await db.execute(sql`
      ...
      WHERE bi.business_id = ${bId} AND bi.date >= ${startDate} AND bi.date <= ${endDate} AND bi.is_special_loan = false
